	...
	<div class="container">
		...
		<form action="#" th:action="@{/lectures/save}"
			...
			<select th:name="courseId">
				<option th:field="*{course}" th:each="course : ${allCourses}" th:value="${course.id}"
					th:utext="${course.name}" />
			</select>
			
			<label>Select classroom</label>
			<select th:name="classroomId">
				<option th:field="*{classRoom}" th:each="classroom : ${allClassrooms}"
					th:value="${classroom.id}" th:utext="${classroom.address}" />
			</select>
			<br>
			<label>Select group</label>
			<select th:name="groupId">
				<option  th:field="*{group}" th:each="group : ${allGroups}" th:value="${group.id}"
					th:utext="${group.name}" />
			</select>
			
			<label>Select teacher</label>
			<select th:name="teacherId">
				<option th:field="*{teacher}" th:each="teacher : ${allTeachers}" th:value="${teacher.id}"
					th:utext="${teacher.lastName}" />
			</select>
			<br>
			<button type="submit" class="btn btn-info col-2">Save</button>

			<hr>
			<a th:href="@{/lectures/showAllLectures}">Back to lectures list</a>
